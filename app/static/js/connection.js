...
const ConnectionView = {
    ...
    init() {
        this.els = {
            ...
            summarizerProviderSelect: document.getElementById("summarizer-provider-select"),
            summarizerBaseUrl: document.getElementById("summarizer-base-url"),
            summarizerApiKey: document.getElementById("summarizer-api-key"),
            summarizerApiKeyGroup: document.getElementById("summarizer-api-key-group"),
            summarizerModelInput: document.getElementById("summarizer-model-input"),
            ...
            summarizerConfigGroup: document.getElementById("summarizer-config-group"),
        ...
        this.els.providerSelect.addEventListener("change", () => this._onProviderChange());
        // Fetch models button
        this.els.fetchModels.addEventListener("click", () => this._fetchModels());
        // Test connection
        this.els.testBtn.addEventListener("click", () => this._testConnection());
        // Start adventure
        this.els.startBtn.addEventListener("click", () => this._startAdventure());
        ...
        if (this.els.npcProviderSelect) {
            this.els.npcProviderSelect.addEventListener("change", () => this._onAgentProviderChange("npc"));
        }
    ...
    _populateAgentDefaults(prefix) {
        ...
        if (modelInput)
            modelInput.value = mainModel;
        if (apiKeyInput)
            apiKeyInput.value = mainApiKey;
    ...
    async _postSettings(payload) {
        try {
            ...
            if (data.ok)
                return data;
            console.warn("Settings POST returned error:", data.error || data.errors);
            return null;
        }
        catch (e) {
        ...
        try {
            ...
            if (data.ok) {
                ...
                if (settingsResult && settingsResult.settings) {
                    this._setStatus("success", `Connected! Latency: ${data.latency_ms.toFixed(1)}ms — Settings saved`);
                }
                else {
                    this._setStatus("success", `Connected! Latency: ${data.latency_ms.toFixed(1)}ms — Model: ${data.model}`);
                }
                ...
                this._setStatus("error", data.error || "Connection failed (no details)");
                this.els.startBtn.disabled = true;
            }
        }
        catch (err) {
    ...
    async _startAdventure() {
        ...
        const settingsPayload = {
            ...
            api_key: apiKey,
            provider_type: this.els.providerSelect.value,
            dm_max_tokens: parseInt(this.els.dmMaxTokens.value, 10) || undefined,
            dm_temperature: parseFloat(this.els.dmTemperature.value) || undefined,
            dm_timeout: parseInt(this.els.dmTimeout.value, 10) || undefined,
            npc_max_tokens: parseInt(this.els.npcMaxTokens.value, 10) || undefined,
            npc_temperature: parseFloat(this.els.npcTemperature.value) || undefined,
            npc_timeout: parseInt(this.els.npcTimeout.value, 10) || undefined,
            summarizer_max_tokens: parseInt(this.els.summarizerMaxTokens.value, 10) || undefined,
            summarizer_temperature: parseFloat(this.els.summarizerTemperature.value) || undefined,
            summarizer_timeout: parseInt(this.els.summarizerTimeout.value, 10) || undefined,
        };
    ...
    _buildAgentProvider(prefix) {
        ...
        if (tempEl)
            config.temperature = parseFloat(tempEl.value) || undefined;
        if (timeoutEl)
            config.timeout = parseInt(timeoutEl.value, 10) || undefined;
        return config;
    },
    // ------------------------------------------------------------------
    // Status display
