...
async function _postJSON(url: string, body: unknown): Promise<Record<string, unknown>> {
    ...
    if (!data.ok) throw new Error(String(data.error || "Request failed"));
    return data;
}

/** Show a modal and optional overlay by removing the "hidden" class. */
function _showModal(modal: HTMLElement, overlay?: HTMLElement | null): void {
    modal.classList.remove("hidden");
    if (overlay) overlay.classList.remove("hidden");
}

/** Hide a modal and optional overlay by adding the "hidden" class. */
function _hideModal(modal: HTMLElement, overlay?: HTMLElement | null): void {
